...
def to_process_request(request, response):
...
def do_save_request(request, response):
    r = Request()
    try:
...
class RequestMiddleware(MiddlewareMixin):
    def process_response(self, request, response):
        if not to_process_request(request, response):
            return response

        do_save_request(request, response)
...
class AsyncRequestMiddleware(MiddlewareMixin):
    def process_response(self, request, response):
        ...
        executor.submit(do_save_request, request, response)
